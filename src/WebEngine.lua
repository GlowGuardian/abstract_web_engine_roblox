...
local LayoutEngine = require(script.Parent.LayoutEngine)
...
function WebEngine.new(config)
	...
	self.styleResolver = StyleResolver.new()
	self.layoutEngine = LayoutEngine.new(self.config.layout)
	return self
end
...
function WebEngine:navigate(url)
	...
	self.styleResolver:compute(document, styles)
	local layout = self.layoutEngine:layout(document)

	self.renderer:render(document, styles, self.linkRouter, layout)
	return document
end
